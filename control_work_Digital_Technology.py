...
def checkLogicalMessage(numberStation, resultCheckFormat):
    ...
    if listMessages[-1][0] == '200' and numberStation > 1 and listMessages[-2][0] != '205' and listMessages[-1][1] == listMessages[-2][1]:
        numberErrors += 1
        fileTXTResult.write(f'Нарушена очередность передачи сообщения 200. Отсутствует сообщение 205. Ошибка {str(numberErrors)}\n')
        skipCheck +=1
    elif listMessages[-1][0] == '202' and numberStation > 1 and (listMessages[-2][0] != '200' and listMessages[-2][0] != '202'):
        numberErrors += 1
        fileTXTResult.write(f'Нарушена очередность передачи сообщения 202. Отсутствует сообщение 200 или 202. Ошибка {str(numberErrors)}\n')
        skipCheck +=1
    elif listMessages[-1][0] == '201' and numberStation == 0:
        numberErrors += 1
        skipCheck +=1
        fileTXTResult.write(f'Нарушена очередность передачи сообщения 201. Отсутствует предшествующее сообщение 200 или 202. Ошибка {str(numberErrors)}\n')
    elif listMessages[-1][0] == '201' and numberStation > 1 and (listMessages[-2][0] != '200' and listMessages[-2][0] != '202'):
        numberErrors += 1
        skipCheck +=1
        fileTXTResult.write(f'Нарушена очередность передачи сообщения 201. Отсутствует сообщение 200 или 202. Ошибка {str(numberErrors)}\n')
    elif listMessages[-1][0] == '205' and numberStation > 1 and (listMessages[-2][0] != '201') and listMessages[-2][1] == listMessages[-1][1]:
        numberErrors += 1
        skipCheck +=1
        fileTXTResult.write(f'Нарушена очередность передачи сообщения 205. Отсутствует сообщение 201. Ошибка {str(numberErrors)}\n')
    ...
    if listMessages[-1][1][:4] not in set(stations[0]):
        numberErrors += 1
        fileTXTResult.write(f'Станция {listMessages[-1][1][:4]} отсутствует в маршруте движения поезда. Ошибка {str(numberErrors)}. Проверка прервана \n')
        skipCheck += 1
    ...
    if ((listMessages[-1][0] == '202' or (listMessages[-1][0] == '205' and numberStation == 0 )  
        ...
        numberErrors += 1
        fileTXTResult.write(f'\t\t Сообщение 200 не может быть первым сообщением о движении поезда. Ошибка {str(numberErrors)}\n')
        skipCheck +=1
        ...
        numberErrors += 1
        skipCheck +=1
        fileTXTResult.write(f'\t\tЧерез станцию {listMessages[-1][1][:4]} поезд уже проезжал. Проверка следующих сообщений пропущена. Ошибка {str(numberErrors)}\n')
    

    if resultCheckFormat == 'ОШИБКА МАКЕТА СООБЩЕНИЯ':
        numberErrors += 1
        fileTXTResult.write(f'\t\tЛогическая проверка сообщения пропущена. Ошибка {str(numberErrors)}\n')
        return numberStation
    
    if skipCheck == 0:    
        if ((listMessages[-1][0] != '202' or listMessages[-1][0] != '205') and numberStation == 0 and len(listMessages) == 1):
            numberErrors += 1
            fileTXTResult.write(f'\t\tЭто сообщение не может быть первым. Ошибка {str(numberErrors)}\n')
        ...
        if listMessages[-1][1] != '0000':
            if ((listMessages[-1][1] !=  stations[0][numberStation-1] and len(listMessages[-1][1]) == 4) 
                ...
                numberErrors += 1
                fileTXTResult.write(f'{listMessages[-1][1]} Станция передачи сообщение НЕ СООТВЕТСТВУЕТ маршруту следования. Ошибка {str(numberErrors)}\n')
            else: 
                pass
                if flagLoging == 1:
                    fileTXTResult.write(f'{stations[0][numberStation-1]} Станция передачи сообщение соответствует маршруту следования\n')
        else:
            numberErrors += 1
            fileTXTResult.write(f'ОШИБКА МАКЕТА. Код ЕСР станции не проверяется. Ошибка {str(numberErrors)}\n')
        ...
        if len(stations[0]) == 2 and listMessages[-1][1][:4] == listMessages[-1][3]:
            if flagLoging == 1:
                fileTXTResult.write(f'\t\tСтанция {listMessages[-1][1][:4]} передачи сообщения соотвествует станции формирования поезда\n')
        elif len(stations[0]) == 2 and listMessages[-1][1][:4] != listMessages[-1][3] and (listMessages[-1][1] == '205' and listMessages[-1][1] == '200'):
            numberErrors += 1
            fileTXTResult.write(f'\t\tСтанция {listMessages[-1][1][:4]} передачи сообщения НЕ СООТВЕСТВУЕТ станции {listMessages[-1][3]} формирования поезда. Ошибка {str(numberErrors)}\n\t\t Проверка следующих сообщений не выполняется. \n' )
            skipCheck += 1
            return(numberStation)
        ...
        if len(stations[0]) == 2 and stations[0][0] == listMessages[-1][3] and stations[0][1] == listMessages[-1][5]:
            if flagLoging == 1:
                fileTXTResult.write(f'\t\tСтанция формирования {listMessages[-1][3]} соответствует станции {stations[0][0]} начала движения поезда.\n')
                fileTXTResult.write(f'\t\tСтанция назначения {listMessages[-1][5]} соответствует станции {stations[0][1]} окончания движения поезда.\n')
        elif len(stations[0]) == 2 and stations[0][0] != listMessages[-1][3]:
            numberErrors += 1
            fileTXTResult.write(f'\t\tСтанция формирования {listMessages[-1][3]} НЕ СООТВЕТСТВУЕТ станции {stations[0][0]} начала движения поезда. Ошибка {str(numberErrors)}\n')
            skipCheck += 1
            return(numberStation)
        elif len(stations[0]) == 2 and stations[0][0] != listMessages[-1][3]:
            numberErrors += 1
            fileTXTResult.write(f'\t\tСтанция назначения {listMessages[-1][5]} НЕ СООТВЕТСТВУЕТ станции {stations[0][1]} окончания движения поезда. Ошибка {str(numberErrors)}\n')
            skipCheck += 1
            return(numberStation)
        ...
        if (len(stations[0]) == 2 and listMessages[-1][1][-4:] ==  stations[0][numberStation-1] and len(listMessages[-1][1]) == 9  
            ...
            pass
            if flagLoging == 1:
                    fileTXTResult.write(f'Станция {listMessages[-1][3]} передает сообщение за станцию {stations[0][numberStation-1]}\n')
        elif (len(stations[0]) == 2 and listMessages[-1][1][-4:] ==  stations[0][numberStation-1] and len(listMessages[-1][1]) != 9  
                and stations[1][stations[0].index(listMessages[-1][3])][:1] == '3' 
                and stations[1][stations[0].index(listMessages[-1][5])][:1] == '3' 
                and (listMessages[-1][0] == '201' or listMessages[-1][0] == '203')):
            numberErrors += 1
            fileTXTResult.write(f'Станция {listMessages[-1][3]} должна передать сообщение за станцию {stations[0][numberStation-1]}. Ошибка {str(numberErrors)}\n')
        ...
        if (listMessages[-1][0] == '202' and stations[1][stations[0].index(listMessages[-1][1])][:1] not in listRole202):
            numberErrors += 1
            fileTXTResult.write(f'\t Стания {listMessages[-1][1]} не может передавать сообщение о проследовании. Ошибка {str(numberErrors)}\n')
            ...
            numberErrors += 1
            fileTXTResult.write(f'\t Стания {listMessages[-1][1]} не может передавать сообщение о работе с поездом на станции. Ошибка {str(numberErrors)}\n')
        ...
        if numberStation == 1: # Если станция первая в списке, проверяем соответствие номера сквозному
            if (listMessages[-1][2][:1] == '2' and quantityTechnStation >= 3  # кол-во сорт и участ ст-ий >= 3
                ...
                pass
                if flagLoging == 1:
                    fileTXTResult.write(f'\t\t Кол-во техн станций на маршруте движения {str(quantityTechnStation)}, кол-во междор стыков {str(quantityBoardStation)} поезд сквозной {listMessages[-1][2]}\n')
            else:
                if flagLoging == 1:
                    fileTXTResult.write(f'\t\t Кол-во техн станций на маршруте движения {str(quantityTechnStation)}, кол-во междор стыков {str(quantityBoardStation)} поезд не сквозной {listMessages[-1][2]} \n')
        else:
            if numberStation > 0:    
                if (listMessages[-2][2] == listMessages[-1][2]
                    ...
                    numberErrors += 1
                    fileTXTResult.write(f'\t\tНомер поезда НЕ СООТВЕТСТВУЕТ предыдущему сообщению. Ошибка {str(numberErrors)}\n')
            else: 
        ...
        if len(stations[0]) == 2 and (listMessages[-1][2][:2] == '36' or listMessages[-1][2][:2] == '35'):
            if flagLoging == 1:
                    fileTXTResult.write(f'\t\t Номер поезда {listMessages[-1][2]} соответствует маршруту его движения\n')
        elif len(stations[0]) == 2 and (listMessages[-1][2][:2] != '36' and listMessages[-1][2][:2] != '35'):
            numberErrors += 1
            fileTXTResult.write(f'\t\t Номер поезда {listMessages[-1][2]} НЕ СООТВЕТСТВУЕТ маршруту его движения. Ошибка {str(numberErrors)}\n')

        if numberStation == 1 and len(stations[0]) == 2 and listMessages[-1][1][:4] != listMessages[-1][3]:
            numberErrors += 1
            fileTXTResult.write(f'\t\tИндекс поезда  НЕ СООТВЕСТВУЕТ СТАНЦИИ ФОРМИРОВАНИЯ. Ошибка {str(numberErrors)}\n')

        if numberStation == 2 and len(stations[0]) == 2 and listMessages[-1][1][-4:] != listMessages[-1][5]:
            numberErrors += 1
            fileTXTResult.write(f'\t\tИндекс поезда  НЕ СООТВЕСТВУЕТ СТАНЦИИ НАЗНАЧЕНИЯ. Ошибка {str(numberErrors)}\n')
        ...
        if numberStation == 1:
            if listMessages[-1][2][:1] == '2' and ((listMessages[-1][3][:2] not in setRegions and listMessages[-1][5][:2] not in setRegions) # ст-ии форм-ия и назн-ия не входят в дорогу 
                ...
                numberErrors += 1
                fileTXTResult.write(f'\t\tНомер поезда НЕ СООТВЕТСТВУЕТ индексу поезда. Ошибка {str(numberErrors)}\n')    
            
            if (listMessages[-1][3] in stationsAllSet and
                stationsAllList[1][stationsAllList[0].index(listMessages[-1][3])][:1] not in listRole200):
                numberErrors += 1
                fileTXTResult.write(f'\t\tСтанция {listMessages[-1][3]} НЕ МОЖЕТ быть станцией формирования поезда. Ошибка {str(numberErrors)}\n')
            
            if (listMessages[-1][5] in stationsAllSet and
                stationsAllList[1][stationsAllList[0].index(listMessages[-1][5])][:1] not in listRole200) :
                numberErrors += 1
                fileTXTResult.write(f'\t\tСтанция {listMessages[-1][5]} НЕ МОЖЕТ быть станцией назначения поезда. Ошибка {str(numberErrors)}\n')
        ...
        if numberStation > 1:
            if compareField(3)==1 and compareField(4)==1 and compareField(5)==1:
                ...
                numberErrors += 1
                fileTXTResult.write(f'\t\tИндекс поезда НЕ СООТВЕТСТВУЕТ индексу поезда в предыдущем сообщении. Ошибка {str(numberErrors)}\n')
        ...
        if (listMessages[-1][0] == '202' and (stations[1][stations[0].index(listMessages[-1][1])][:1] != '4' and stations[1][stations[0].index(listMessages[-1][1])][:1] != '3')
            ...
            numberErrors += 1
            fileTXTResult.write(f'\t\tСтанцция {listMessages[-1][1]} НЕ ЯВЛЯЕТСЯ ПРОМЕЖУТОЧНОЙ УЗЛОВОЙ. Указывается только направление ОТКУДА. Ошибка {str(numberErrors)}\n')
            ...
            pass
            if flagLoging == 1:
                fileTXTResult.write(f'\t\t Станцция {listMessages[-1][1]} узловая \n')
        ...
        if listMessages[-1][0] == '205' or listMessages[-1][0] == '200':
                ...
                if listMessages[-1][6] == codeStationWhere or listMessages[-1][6][:2] not in setRegions:
                    pass
                    if flagLoging == 1:
                        fileTXTResult.write(f'\t\t {listMessages[-1][6]} Напр. следования поезда КУДА соответствует маршруту\n')
                else:
                    numberErrors += 1
                    fileTXTResult.write(f'\t\t{listMessages[-1][6]} Напр. следования поезда КУДА НЕ СООТВЕТСТВУЕТ маршруту. Ошибка {str(numberErrors)}\n\n')
        ...
        # с. 201
            ...
            if listMessages[-1][6] == codeStationWhereFrom or listMessages[-1][6][:2] not in setRegions:
                pass
                if flagLoging == 1:
                    fileTXTResult.write(f'\t\t{listMessages[-1][6]} Напр. следования поезда ОТКУДА соответствует маршруту\n')
            else:
                numberErrors += 1
                fileTXTResult.write(f'\t\t{listMessages[-1][6]}  Напр. следования поезда ОТКУДА НЕ СООТВЕТСТВУЕТ маршруту. Ошибка {str(numberErrors)}\n\n')
        ...
        # с. 202
            ...
            if numberStation >= 1: # сообщение первое и далее в описании движения поезда
                ...
                if listMessages[-1][6][:4] == codeStationWhereFrom:
                    ...
                    numberErrors += 1
                    fileTXTResult.write(f'\t\t {listMessages[-1][6][:4]} Напр. следования поезда ОТКУДА НЕ СООТВЕТСТВУЕТ маршруту. Ошибка {str(numberErrors)}\n')
                if stations[1][stations[0].index(listMessages[-1][1])][:1] == '4':
                    ...
                    if listMessages[-1][6][5:] == findCodeStThroughRoleWhereFrom(listMessages[-1][6][5:]):
                        pass
                        if flagLoging == 1:
                            fileTXTResult.write(f'\t\t {listMessages[-1][6][5:]} Напр. следования поезда КУДА соответствует маршруту\n')
                    else:
                        numberErrors += 1
                        fileTXTResult.write(f'\t\t{listMessages[-1][6][5:]} Напр. следования поезда КУДА НЕ СООТВЕТСТВУЕТ маршруту. Ошибка {str(numberErrors)}\n')
            ...
            if len(listMessages[-1][6]) == 8:
                ...
                numberErrors += 1
                fileTXTResult.write(f'\t\t № вагона должен быть из восьми цифр. Ошибка {str(numberErrors)}\n')
        ...
        if listMessages[-1][0] == '205' and numberStation != 1 and listMessages[-2][0] == '201' and stations[1][stations[0].index(listMessages[-1][1])][:1] == '1':
            ...
            if hoursParking*60+minutesParking < 40:
                numberErrors += 1
                fileTXTResult.write(f'\t\t {str(hoursParking*60+minutesParking)} мин. Время стоянки поезда на сорт. станции меньше 40 минут. Ошибка {str(numberErrors)}\n')
            else:
                pass
                if flagLoging == 1:
                    fileTXTResult.write(f'\t\t {str(hoursParking*60+minutesParking)} Время стоянки поезда на сорт. станции \n')
        ...
        if listMessages[-1][0] == '205' and numberStation != 1 and listMessages[-2][0] == '201' and stations[1][stations[0].index(listMessages[-1][1])][:1] == '2':
            ...
            if hoursParking*60+minutesParking < 15:
                numberErrors += 1
                fileTXTResult.write(f'\t\t{str(hoursParking*60+minutesParking)} Время стоянки поезда на участ. станции меньше 15 минут. Ошибка {str(numberErrors)}\n')
            else:
                pass
                if flagLoging == 1:
                    fileTXTResult.write(f'\t\t{str(hoursParking*60+minutesParking)} Время стоянки поезда на участ. станции \n')
        ...
        if numberStation > 1 and listMessages[-1][0] == '200' and listMessages[-2][0] == '205': # and (stations[1][stations[0].index(listMessages[-1][1])][:1] == '2'):
            ...
            if hoursParking*60+minutesParking > 10:
                numberErrors += 1
                fileTXTResult.write(f'\t\t{str(hoursParking*60+minutesParking)} мин. Время стоянки поезда от готовности до отправления больше 10 минут. Ошибка {str(numberErrors)}\n')
            else:
                pass
                if flagLoging == 1:
                    fileTXTResult.write(f'\t\t{str(hoursParking*60+minutesParking)} Время стоянки поезда от готовности до отправления\n')
        ...
        if listMessages[-1][0] == '203' and numberStation != 1 and listMessages[-2][0] == '201': # and stations[1][stations[0].index(listMessages[-1][1])][:1] == '2':
            ...
            if hoursParking*60+minutesParking < 15:
                numberErrors += 1
                fileTXTResult.write(f'\t\t{str(hoursParking*60+minutesParking)} мин. Время стоянки поезда от прибытия до расформирования меньше 15 минут. Ошибка {str(numberErrors)}\n')
            else:
                pass
                if flagLoging == 1:
                    fileTXTResult.write(f'\t\t{str(hoursParking*60+minutesParking)} мин. Время стоянки поезда от прибытия до расформирования\n')
        ...
        if listMessages[-1][0] == '205' and numberStation != 1 and listMessages[-2][0] == '201' and stations[1][stations[0].index(listMessages[-1][1])][:1] == '2':
            ...
            if hoursParkingBrigade*60+minutesParkingBrigade > 120:
                numberErrors += 1
                fileTXTResult.write(f'\t\t{str(hoursParkingBrigade*60+minutesParkingBrigade)} мин. Время работы лок. бригады до готовности больше 120 минут. Ошибка {str(numberErrors)}\n')
            else:
                pass
                if flagLoging == 1:
                    fileTXTResult.write(f'\t\t {str(hoursParkingBrigade*60+minutesParkingBrigade)} мин. Время работы лок. бригады до готовности\n')
        ...
        if listMessages[-1][0] != '203':
            ...
            if numberStation != 1 and listMessages[-1][0] == '205' and listMessages[-2][12] == '1' and compareField(14) == 0: # compareField(13) == 0 and 
                pass
                if flagLoging == 1:
                ...
                numberErrors += 1
                fileTXTResult.write(f'\t Смена лок-ва НЕ ВЫПОЛНЕНА. ОШИБКА {str(numberErrors)} \n')
            elif numberStation != 1 and listMessages[-1][0] == '205' and (listMessages[-2][12] == '0' or listMessages[-2][12] == '2'): #  and compareField(14) == 1:
                pass
                if flagLoging == 1:
            ...
            if numberStation == 1 and (listMessages[-1][0] == '202' or listMessages[-1][0] == '205' or listMessages[-1][0] == '200'):
                if listMessages[-1][14][:-1] == '0000':
                    numberErrors += 1
                    fileTXTResult.write(f'\t Номер лок-ва {listMessages[-1][14]} НЕ ДОПУСТИМ. ОШИБКА {str(numberErrors)} \n')
                if listMessages[-1][14][-1:] == '0' or listMessages[-1][14][-1:] == '1' or listMessages[-1][14][-1:] == '2':
                    ...
                    numberErrors += 1
                    fileTXTResult.write(f'\t\tПоследний символ № лок-ва НЕ ВЕРЕН. Ошибка {str(numberErrors)}\n\t\t Далее номера лок-ов не проверяются.\n')
                ...
                pass
                if flagLoging == 1:
                    fileTXTResult.write('\t Серия и Номер лок-ва соответствует предыдущему сообщению\n')
            ...
            if listMessages[-1][15] == '1':
                pass
                if flagLoging == 1:
                    fileTXTResult.write(f'\t {listMessages[-1][15]} Вид следования лок-ва соответствует <<В ГОЛОВЕ>> \n')
            else:
                numberErrors += 1
                fileTXTResult.write(f'\t{listMessages[-1][15]} Вид следования лок-ва  НЕ СООТВЕТСТВУЕТ <<В ГОЛОВЕ>>. Ошибка {str(numberErrors)}\n')
            ...
            if hoursWork <= 12:
                ...
                numberErrors += 1
                fileTXTResult.write(f'\t{str(hoursWork)} час. {str(minutesWork)} мин. Время работы лок. бригады ПРЕВЫШАЕТ 12 ЧАСОВ. Ошибка {str(numberErrors)}\n')

            # проверка депо приписки
            if listMessages[-1][18][-2:] != '00':
                if flagLoging == 1:
                    fileTXTResult.write(f'\t{listMessages[-1][18]} Номер депо приписки лок. бригады допустим \n')
            else:
                numberErrors += 1
                fileTXTResult.write(f'\t{listMessages[-1][18]} Номер депо приписки лок. бригады НЕ ДОПУСТИМ. Ошибка {str(numberErrors)}\n')
            ...
            if numberStation == 1:
                if listMessages[-1][0] == '202' and stations[1][stations[0].index(listMessages[-1][1])][:1] == '5':
                    if listMessages[-1][18][:2] == min(list(setRegions)):
                        pass
                        if flagLoging == 1:
                            fileTXTResult.write(f'\t {listMessages[-1][18]} Депо приписки лок. бригады заданной дороги \n')
                    elif listMessages[-1][18][:2] not in setRegions:
                        pass
                        if flagLoging == 1:
                            fileTXTResult.write(f'\t{listMessages[-1][18]} Депо приписки лок. бригады не заданной дороги \n')
                    else:
                        numberErrors += 1
                        fileTXTResult.write(f'\t{listMessages[-1][18]} Депо приписки лок. бригады НЕ СООТВЕТСВУЕТ заданой дороге. Ошибка {str(numberErrors)}\n')
                ...
                if numberStation > 1 and (listMessages[-1][0] == '202' or listMessages[-1][0] == '201' or listMessages[-1][0] == '200') and compareField(18) == 1:
                    pass
                    if flagLoging == 1:
                        fileTXTResult.write(f'\t {listMessages[-1][18]} Депо приписки лок. бригады СООТВЕТСВУЕТ предыдущему сообщению \n')
                elif listMessages[-1][0] == '205' and listMessages[-2][0] == '201' and listMessages[-1][18][:2] == min(list(setRegions)):
                    pass
                    if flagLoging == 1:
                        fileTXTResult.write(f'\t {listMessages[-1][18]} Депо приписки лок. бригады нашего региона \n')
                elif listMessages[-1][0] == '205' and listMessages[-2][0] == '201' and listMessages[-1][18][:2] != min(list(setRegions)):
                    numberErrors += 1
                    fileTXTResult.write(f'\t{listMessages[-1][18]} Депо приписки лок. бригады НЕ СООТВЕТСТВЕТ заданной дороге. Ошибка {str(numberErrors)}\n\tСледующие информационные фразы об этой лок. бригаде не проверяются.\n\n')
                else:
                    numberErrors += 1
                    fileTXTResult.write(f'\t{listMessages[-1][18]} Депо приписки лок. бригады НЕ СООТВЕТСВУЕТ {listMessages[-2][18]} предыдущему сообщению. Ошибка {str(numberErrors)}\n\n')
            ...
            if (listMessages[-1][0] == '205' and numberStation != 1 and stations[1][stations[0].index(listMessages[-1][1])][:1] == '1'
                ...
                pass
                if flagLoging == 1:
                    fileTXTResult.write(f'\t {listMessages[-1][19]} {listMessages[-1][20]} Смена лок. бригады выполнена\n\n')
            elif ((listMessages[-1][0] != '205' and listMessages[-1][0] != '203') and numberStation != 1 
                and compareField(19) == 1 and compareField(20) == 1):
                pass
                if flagLoging == 1:
                    fileTXTResult.write(f'\t {listMessages[-1][19]} {listMessages[-1][20]} Лок. бригада соответствует предыдущему сообщению\n')
            elif (listMessages[-1][0] == '205' and numberStation > 1 
                and (compareField(19) == 0 or compareField(20) == 0)):
                pass
                if flagLoging == 1:
                    fileTXTResult.write(f'\t {listMessages[-1][19]} {listMessages[-1][20]} Лок. бригада сменена\n')
            
            elif ((listMessages[-1][0] != '205' or listMessages[-1][0] != '203' ) and numberStation > 1
                and (compareField(19) == 0 or compareField(20) == 0)):
                numberErrors += 1
                fileTXTResult.write(f'\t{listMessages[-1][19]} {listMessages[-1][20]} Лок. бригада НЕ СООТВЕТСТВУЕТ {listMessages[-2][19]} {listMessages[-2][20]} предыдущему сообщению. Ошибка {str(numberErrors)}\n')
...
def compareField(numberField:int):
    """
    Сравнение поля с номером numberField последней и предпоследней записей в списке сообщений. Номера полей:
    0 - код сообщения;
    1 - код пункта передачи;
    2 - номер поезда;
    3 - код станции формирования состава;
    4 - порядковый номер состава;
    5 - код станции назначения состава;
    6 - код станции направления движения поезда;
    7, 8 - дата выполнения операции;
    9, 10 - время выполнения операции;
    11 - код парка и пути (при наличии);
    12 - вид работы с лок-ом и лок. бригадой с.201;
    13 - код серии лок-ва;
    14 - номер лок-ва;
    15 - вид следования лок-ва;
    16, 17 - время явки лок. бригады; 
    18 -  код депо приписки лок. бригады;
    19 - таб. номер лок. бригады;
    20 - фамилия машиниста.
    """
...
for nameFileTXT in fileList:
    if nameFileTXT[-3:] != 'txt': # открываем файлы только с расширением TXT
        ...
        fileTXTResult = open(nameFileTXTResult, 'w', encoding = 'cp1251') # результат проверки, encoding='cp1251'
        # обработка номеров вагонов
        for i in fileTXT:
            lineTXT = i.strip()
            if len(lineTXT) != 8 and lineTXT != '' and lineTXT != '#':
                numberErrors += 1
                fileTXTResult.write(f'{lineTXT} не является номером вагона. Нарушен формат файла. Проверка прервана. Ошибка {str(numberErrors)}\n')
                stopCheck += 1
                break
            if lineTXT == '':
                continue
            if lineTXT == '#':
                ...
                if res == 'НОМЕР ВАГОНА НЕ ВЕРЕН':
                    numberErrors += 1
                    fileTXTResult.write(f'{lineTXT} {res}. Ошибка {str(numberErrors)}\n')
                else:
                    fileTXTResult.write(f'{lineTXT} {res}\n')
        ...
        for i in fileTXT:
            ...
            if lineTXT == '':
                ...
                numberErrors += 1
                fileTXTResult.write(f'{lineTXT} Формат строки НЕ ВЕРЕН. Проверка прервана. Ошибка {str(numberErrors)}\n')
                stopCheck += 1
                break
            else:
                ...
                if len(dictStation) == 1:
                    ...
                    if lineTXT[0] != '5':
                        setRegions: set[str] =  {lineTXT[3:7][:2]}
                else:
                ...
                if len(dictStation) != len(setStation):
                    numberErrors += 1
                    fileTXTResult.write(f'{lineTXT} \t\t\tкод станции {lineTXT[3:7]} повторяется. Ошибка {str(numberErrors)}\n')
                ...
                if resStation == 'Код ЕСР станции НЕ ВЕРЕН':
                    numberErrors += 1
                    fileTXTResult.write(f'{lineTXT} {resStation}. Ошибка {str(numberErrors)} \n')
                else:
                    fileTXTResult.write(f'{lineTXT} {resStation} \n')
        ...
        for i in fileTXT:
            ...
            if lineTXT == '':
                ...
                for station in stations[0]:
                    if station in setStation:
                        ...
                        fileTXTResult.write(f'{station} СТАНЦИЯ ОТСУТСТВУЕТ В СПИСКЕ СТАНЦИЙ ДОРОГИ. Проверка сообщений этого поезда прервана. Ошибка {str(numberErrors)}\n')
                        stopCheck +=1
                        break
            ...
            if stopCheck > 0 and lineTXT[0] != '!': # пропускаем обработку следующего описания поезда
                ...
                if messageSPhrase[0] == '203': # пополнение с.203 до полного сообщения
                    message = addingMessage(messageSPhrase.copy()) + messageIPhraseEmpty.copy()
                elif messageSPhrase[0] == '201' or messageSPhrase[0] == '202' or messageSPhrase[0] == '200': # пополнение информационной фразой из предыдущего сообщения
                    message = addingMessage(messageSPhrase.copy()) + listMessages[-1][13:].copy()
                else:
                    fileTXTResult.write(f'{lineTXT}\t\t\t Для этого сообщения должна быть информационная фраза\n\n')
                messageSPhrase.clear()
            ...
            elif lineTXT[:2] != '(:'and lineTXT[-2:] == ':)': # информационная фраза со служебной в предыд. строке
                messageTemp = lineTXT[:-2].split()
                if len(messageSPhrase) == 0:
                    fileTXTResult.write(f'{lineTXT} \t\tДля информационной фразы отсутствует служебная фраза. Проверка прервана.\n')
                    skipCheck += 1
                    continue
                ...
                if resultCheckFormatIPhrase == 'МАКЕТ ВЕРЕН':
                    ...
                    messageIPhrase = messageTemp.copy()
                fileTXTResult.write(f'{lineTXT}\t\t\t{resultCheckFormatIPhrase}\n\n')
                ...
                fileTXTResult.write(f'{lineTXT}\t\tПроверка сообщения пропущена\n')
        ...
        if countTrain < 5:
            numberErrors += 1
            fileTXTResult.write(f'Описано продвижение не всех поездов. Должно быть 5, описано {str(countTrain)} ОШИБКА {str(numberErrors)}.\n')
        fileTXT.close()
        fileTXTResult.close()
        encoding='cp1251'
        if numberErrors != 0:
            nameFileTXTResultErr = nameFileTXTResult[:-6]+'Err_' + str(numberErrors) + '.txt'
