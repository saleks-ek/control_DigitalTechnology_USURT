...
def checkLogicalMessage(numberStation, numberErrors, stopCheck):
    """
    ...
    if listMessages[-1][1][:4] not in set(stations[0]):
        ...
        return numberStation, numberErrors, stopCheck
    ...
    if listMessages[-1][0] != '203':
        ...
        if numberStation == 1:
            ...
            if numberStation > 1 and (listMessages[-1][0] == '202' or listMessages[-1][0] == '201' or listMessages[-1][0] == '200') and compareField(18) == 1:
                ...
                fileTXTResult.write('\t ' + listMessages[-1][18] + ' Депо приписки лок. бригады НЕ СООТВЕТСТВЕТ нашей дороге \n')
            else:
    ...
    if nameFileTXT[-3:] != 'txt': # открываем файлы только с расширением TXT
        ...
        for i in fileTXT:
            ...
            if lineTXT == '':
                ...
                numberStation, numberErrors, stopCheck = checkLogicalMessage(numberStation, numberErrors, stopCheck)
